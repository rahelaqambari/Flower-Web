...
function Testemonial() {
 ...
 responsive: [
  ...
  return (
    <div className='lg:h-[70vh] md:h-[47vh] h-[40vh] w-full flex justify-end items-center my-5 lg:px-8 px-0 bg-cover bg-no-repeat bg-center bg-[url(/images/hero6.jpg)] '>
        <div className='md:w-[600px] w-full h-full md:bg-white bg-black/45 p-6'>
          <Slider {...settings}>
            ...
            <div key={index} className='flex test flex-col justify-center items-cente h-full w-full gap-4 border border-gray-200 relative'>
                ...
                <h3 className='mx-auto md:text-black text-white'>{items.Job}</h3>
            </div>
            ))}
            </Slider>
        </div>
